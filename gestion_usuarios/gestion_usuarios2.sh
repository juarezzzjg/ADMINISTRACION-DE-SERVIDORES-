...
asignar_sudo() {
    local usuario=$1
    read -p "¿El usuario debe poder ejecutar comandos con sudo? (s/n): " respuesta
    test "$respuesta" != "s" && test "$respuesta" != "S" && return

    read -p "¿Qué comandos específicos desea que pueda ejecutar el usuario con sudo? (separados por espacio): " comandos

    echo "$usuario ALL=(ALL) NOPASSWD: $comandos" | sudo tee -a /etc/sudoers > /dev/null
    echo "El usuario '$usuario' ahora puede ejecutar los siguientes comandos con sudo: $comandos"
}
